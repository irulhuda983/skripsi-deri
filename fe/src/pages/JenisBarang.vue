  ...
  <main class="box-border w-full px-4">
              ...
              <tr v-for="(item, i) in jenisBarang" :key="i">
                ...
                <td class="p-2">
                  <div class="text-center flex space-x-2 items-center justify-center">
                    <button class="btn bg-indigo-500 hover:bg-indigo-600 text-white" @click.prevent="openModalUpdate(item.id)">
                    ...
                    <button class="btn bg-red-500 hover:bg-red-600 text-white" @click.prevent="openModalDelete(item.id)">
    ...
    <Modal ref="modalUpdate">
      <template v-slot:modal-body>
        <div class="w-full bg-white">
          <div class="w-full">
            <div class="border-b font-semibold text-xl box-border w-full p-4">Ubah Jenis Barang</div>
            <div class="w-full box-border p-4">
              <div class="flex items-center justify-between">
                <label for="jenis_barang" class="block text-sm font-medium leading-6 text-gray-900">Jenis Barang</label>
                <!-- <div class="text-sm">
...
export default {
  ...
  setup() {
    ...
    const updateData = async() => {
      ...
      loadingSave.value = true;
      try {
        await jenisBarangRepo.update(idJenis.value, payload);
    ...
    const deleteData = async () => {
      try {
        const res = await jenisBarangRepo.delete(idJenis.value);
    ...
    const openModalUpdate = async (id) => {
      try {
        const { data } = await jenisBarangRepo.detail(id)

        payload.jenis_barang = data.data.jenis_barang
